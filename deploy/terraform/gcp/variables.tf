...
# the heavy image (Dockerfile.worker). The server Cloud Run service uses
# the server image; the worker service AND the CronJobs (which share the
# heavy preview-tool runtime) use the worker image.
